# 🎓 Digital Literacy Portfolio – CSE0001

**VIT Bhopal University | VITyarthi E-Learning Platform**

---

## 👤 Student Details

| Field | Details |
|-------|---------|
| **Name** | Kunal Kumar |
| **Registration No.** | 25BCE10196 |
| **Branch** | CSE core |
| **Year** | First Year B.Tech |
| **Course Code** | CSE0001 – Digital Literacy |
| **Academic Year** | 2025–26 |

---

## 📋 Project Overview

This repository contains my **Digital Literacy Portfolio** submitted as part of the CSE0001 course at VIT Bhopal University. As a Student Digital Ambassador, I have completed five tasks covering infographic design, professional profile building, coding platforms, email etiquette, and cybercrime awareness.

---

## 📁 Repository Structure

```
digital-literacy-project/
├── README.md                          
├── report(main)/
│   └── Project_Report.docx            ← Full project report
├── task-1-presentation/
│   ├── description.md                 ← Tool used & reflection
│   └── [infographic screenshot/link]  ← Canva/Prezi export
├── task-2-portfolio/
│   ├── reflection.md                  ← Platform reflection
│   └── [screenshots of profiles]
├── task-3-platforms/
│   ├── reflection.md                  ← Platforms reflection
│   ├── [hackerrank_screenshot]
│   └── [google_form_screenshot]
├── task-4-email-etiquette/
│   ├── emails.md                      ← Two professional emails
│   └── social-media-checklist.md      ← Do's & Don'ts checklist
└── task-5-cybercrime/
    ├── casestudy.md                   ← Phishing case study
    └── prevention-checklist.md        ← Stay Safe Online checklist
```

---

## 📌 Module Summaries

### Task 1 – Digital Literacy Awareness Infographic (Module 1)
Created a one-page visual infographic using **Canva** covering what digital literacy is, safe internet practices, and professional online presence. The design targets first-year students who are navigating the digital world for the first time.

🔗 **Canva Link:** https://bit.ly/4bDYltR

---

### Task 2 – Student Digital Portfolio (Module 2)
Set up professional profiles on **GitHub**, **LinkedIn**, and **Kaggle**. Each platform serves a unique purpose — GitHub for code, LinkedIn for professional networking, and Kaggle for data science practice.

🔗 **GitHub Profile:** https://github.com/kunalgit23113
🔗 **LinkedIn Profile:** https://www.linkdin.com/in/kunal23113
🔗 **Kaggle Profile:** https://www.kaggle.com/kunal196

---

### Task 3 – Coding & Collaboration Platforms (Module 3)
Completed a beginner challenge on **HackerRank** and built a 5-question **Digital Literacy Awareness Quiz** using Google Forms.

🔗 **Google Form (Quiz):** https://bit.ly/40NDFKX
🔗 **HackerRank Profile:** https://www.hackerrank.com/profile/kunal202425

---

### Task 4 – Email Etiquette (Module 4)
Drafted two professional emails — one to a professor requesting a deadline extension, and one to an internship coordinator. Also created a Social Media Do's and Don'ts checklist.

📄 See: [`task-4-email-etiquette/emails.md`](task-4-email-etiquette/emails.md)

---

### Task 5 – Cybercrime Awareness (Module 5)
Researched **Phishing** as a cybercrime type, wrote a detailed case study, and compiled an 8-point prevention checklist tailored for college students in India.

📄 See: [`task-5-cybercrime/casestudy.md`](task-5-cybercrime/casestudy.md)

🆘 **Report cybercrime at:** [cybercrime.gov.in](https://cybercrime.gov.in) | Helpline: **1930**

---

## 🔗 Useful Links

| Resource | URL |
|----------|-----|
| National Cyber Crime Portal | https://cybercrime.gov.in |
| Cyber Crime Helpline | 1930 (24x7) |
| GitHub | https://github.com |
| LinkedIn | https://linkedin.com |
| Canva | https://canva.com |
| HackerRank | https://hackerrank.com |

---

*Submitted via VITyarthi portal as part of CSE0001 – Digital Literacy*

